...
from api.schemas import Chunk, FileContext
...
GET_FILE_CONTEXT_DESCRIPTION = (
    "What the Kalisio corpus knows about one file before you change it. "
    "Call this BEFORE editing the signature, the exports or the behaviour "
    "of an existing file. Returns the files that import it, the files that "
    "historically change in the same commits -- coupling that no import "
    "declares, which a search cannot find -- how many commits touched it, "
    "and its recent commit subjects. Give the repository name and the path "
    "inside it, as `search_code` returns them. `dependent_count` is exact "
    "even when the list is truncated; `indexed: false` means the corpus "
    "does not hold that file, which is not the same as nothing depending "
    "on it."
)
...
def build_server():
    ...
    # Reads what the ingestion job stored about the file -- the graph, the
    # co-changes, the history -- the one question no amount of semantic
    # search can answer.
    @server.tool(description=GET_FILE_CONTEXT_DESCRIPTION)
    def get_file_context(
        repo: Annotated[str, Field(min_length=1, max_length=100)],
        path: Annotated[str, Field(min_length=1, max_length=500)],
    ) -> FileContext:
        return file_context.get_file_context(repo, path)
