...
class TestApi:
    ...
    # Replace the external backends (embedding model, Qdrant, LLM) with stubs
    # so the /ask and /search tests exercise routing + handler logic without
    # needing real services.
    @pytest.fixture
    def stub_backends(self, monkeypatch):
        chunk = {
            "source_path": "kano/store/layers.js", "repository": "kano",
            "breadcrumb": "layers > base", "chunk_index": 0,
            "content": "export const base = []", "commit_history": [],
            "score": 0.9,
        }
        monkeypatch.setattr(handlers, "get_config", lambda: SimpleNamespace(
            top_k=5, max_context_chars=14000,
            prompt_template="Context:\n{context}\n\nQuestion: {question}"))
        monkeypatch.setattr(handlers.embeddings, "encode", lambda text: [0.0])
        monkeypatch.setattr(
            handlers.vectordb, "search", lambda vector, top_k=5: [chunk])
    ...
    def test_ask_with_valid_token_is_accepted(self, auth_on, stub_backends):
    ...
    def test_search_with_valid_token_is_accepted(self, auth_on, stub_backends):
        headers = {"Authorization": f"Bearer {make_token()}"}
        response = client.post(
    ...
    def test_auth_disabled_allows_no_token(self, monkeypatch, stub_backends):
        monkeypatch.setattr(
