...
from config import env_int, env_str, require
...
def test_ingestion_config_is_cached(ingestion_env):
    ...
    config = ingestion_env()
    assert ingestion_config.get_ingestion_config() is config


def test_ingestion_config_requires_the_development_dir(
